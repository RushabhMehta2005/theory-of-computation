...
class TransitionFunction:
    ...
    def add_transition(self, current_state: int, symbol: str, next_state: int) -> None:
        """
        ...
        if symbol not in self.alphabet_set:
            raise ValueError(f"Symbol '{symbol}' is not part of the defined AlphabetSet.")
        if (current_state, symbol) in self.transitions:
            raise ValueError(f"Transition for state {current_state} and symbol '{symbol}' already exists.")
